...
export function TransitSegmentEditorModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100vw-2rem)] max-w-md sm:max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl border-0 shadow-2xl bg-card p-0">
        <DialogHeader className="p-4 sm:p-5 border-b border-border/40 bg-muted/20">
          ...
          <DialogTitle className="text-base sm:text-lg font-bold font-display mt-1">
        ...
        <div className="p-4 sm:p-5 space-y-4 sm:space-y-5 min-w-0">
          {/* Summary Badges */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 rounded-xl bg-muted/40 border border-border/50 text-xs font-semibold gap-2">
          ...
          <div className="space-y-2 min-w-0">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                variant="outline"
                onClick={addSegment}
                className="h-7 text-[11px] gap-1 rounded-lg border-dashed shrink-0"
              >
              ...
              {segments.map((seg, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`p-2.5 sm:p-3 rounded-xl border transition-all cursor-pointer flex items-center justify-between gap-2 sm:gap-3 min-w-0 ${
                      isSelected
                    ...
                    <div className="flex items-center gap-2 sm:gap-2.5 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className="text-xs font-bold truncate min-w-0">{seg.title || option.label}</span>
                          <Badge variant="secondary" className="text-[9px] h-4 px-1 capitalize shrink-0">
                            {seg.type.replace("_", " ")}
                          </Badge>
                        </div>
          ...
          {segments[activeIdx] && (
            <div className="p-3 sm:p-4 rounded-xl border border-border/60 bg-muted/20 space-y-3 min-w-0">
              <h5 className="text-xs font-bold flex items-center gap-1.5 text-primary">
                <Sparkles className="w-3.5 h-3.5 shrink-0" /> Edit Leg #{activeIdx + 1}
              </h5>

              {/* Transit Type Grid */}
              <div className="min-w-0">
                <Label className="text-[10px] font-semibold text-muted-foreground">Mode of Transit</Label>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 mt-1 min-w-0">
                  {TRANSIT_MODE_OPTIONS.map((opt) => (
                    <button
                      key={opt.type}
                      type="button"
                      onClick={() => updateSegment(activeIdx, { type: opt.type })}
                      className={`p-2 rounded-lg text-center transition-all flex flex-col items-center border min-w-0 ${
        ...
        <DialogFooter className="p-4 border-t border-border/40 bg-muted/10 flex flex-col-reverse sm:flex-row gap-2">
          <Button variant="outline" size="sm" onClick={() => onOpenChange(false)} className="w-full sm:w-auto">
            Cancel
          </Button>
          <Button size="sm" onClick={handleSave} className="w-full sm:w-auto gap-1 font-semibold">
